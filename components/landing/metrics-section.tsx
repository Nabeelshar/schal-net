...
const metrics = [
  {
    ...
    suffix: " GB",
    prefix: "",
    label: "Data streamed today",
    sublabel: "across 500+ homes",
  },
  {
    value: 99,
    suffix: ".9%",
...
function AnimatedNumber({ end, suffix = "", prefix = "" }: { end: number; suffix?: string; prefix?: string }) {
  ...
  return (
    <div ref={ref} className="inline-flex items-baseline flex-wrap">
  ...
  return (
    <section ref={sectionRef} className="relative py-20 lg:py-40 overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12">
        {/* Header */}
        <div className="grid lg:grid-cols-12 gap-6 mb-16 lg:mb-32">
          <div className="lg:col-span-8 lg:col-start-1">
            <div className="flex flex-wrap items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
              <span className="inline-flex items-center gap-2 px-3 py-1 bg-[#eca8d6]/10 text-[#eca8d6] text-xs font-mono">
            ...
            <h2 className={`text-[clamp(2.8rem,8vw,9rem)] font-display tracking-tight leading-[0.95] transition-all duration-1000 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}>
              Real-time
              <br />
              <span className="text-muted-foreground">network metrics.</span>
            </h2>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {/* Large metric */}
          <div className={`sm:col-span-2 lg:col-span-1 bg-foreground/[0.02] border border-foreground/10 p-6 sm:p-10 lg:p-14 transition-all duration-700 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
          }`}>
            <div className="text-3xl sm:text-4xl lg:text-5xl font-display tracking-tight mb-4 overflow-hidden">
            ...
            <div className="text-base sm:text-lg text-foreground mb-2">{metrics[0].label}</div>
          ...
          {metrics.slice(1).map((metric, index) => (
            <div
              key={metric.label}
              className={`bg-foreground/[0.02] border border-foreground/10 p-6 sm:p-8 flex flex-col items-start justify-between gap-4 sm:gap-6 transition-all duration-700 ${
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
              }`}
            ...
            >
              ...
              <div className="text-2xl sm:text-3xl lg:text-5xl font-display tracking-tight w-full">
        ...
        {/* Bottom ticker */}
        <div className={`mt-10 sm:mt-16 pt-6 sm:pt-8 border-t border-foreground/10 flex flex-wrap items-center gap-x-6 sm:gap-x-12 gap-y-3 sm:gap-y-4 text-sm font-mono text-muted-foreground transition-all duration-1000 delay-500 ${
          isVisible ? "opacity-100" : "opacity-0"
        }`}>
          <span>FTTH Optical Fiber</span>
          <span>Unlimited Data</span>
          <span>Power Backup</span>
          <span>Low-Latency Routing</span>
          <span className="text-foreground">+ 24/7 Local Support</span>
        </div>
      </div>
    </section>
  );
