...
import { Youtube, MonitorPlay, Gamepad2, Gamepad, Video, Wallet, Phone, Home } from "lucide-react";
...
const integrations: Integration[] = [
  { name: "YouTube (4K)", category: "Streaming", icon: <Youtube className="w-6 h-6" /> },
  { name: "Netflix (Ultra HD)", category: "Streaming", icon: <MonitorPlay className="w-6 h-6" /> },
  { name: "PUBG Mobile (Low Ping)", category: "Gaming", icon: <Gamepad2 className="w-6 h-6" /> },
  { name: "PlayStation Network", category: "Gaming", icon: <Gamepad className="w-6 h-6" /> },
  { name: "Zoom Video", category: "Work", icon: <Video className="w-6 h-6" /> },
  { name: "SadaPay / JazzCash", category: "Bill Pay", icon: <Wallet className="w-6 h-6" /> },
  { name: "WhatsApp Calls", category: "Comms", icon: <Phone className="w-6 h-6" /> },
  { name: "Smart Home", category: "IoT", icon: <Home className="w-6 h-6" /> },
];
    ...
    <section id="integrations" ref={sectionRef} className="relative overflow-hidden">
      ...
      <div className="relative z-10 pt-32 lg:pt-40 text-center">
        ...
        <p className={`mt-8 text-xl text-muted-foreground leading-relaxed max-w-lg mx-auto transition-all duration-1000 delay-100 ${
          ...
          One fiber line for the whole home. Stream, game, video call, and pay your bills without a single buffer.
        </p>
      </div>
        ...
        <div className={`flex flex-wrap items-center justify-between gap-8 pt-12 border-t border-foreground/10 transition-all duration-1000 delay-500 pb-32 lg:pb-40 ${
          ...
          <div className="flex flex-wrap gap-12">
            {[
              ...
              { value: "Low-Ping", label: "Gaming optimized" },
            ].map((stat) => (
